    ...
    <section id="about" ref={ref} className="px-[7.5vw] py-20 lg:py-28 overflow-hidden relative">
          ...
          <h2 className="fontone" style={{ fontWeight: 900, fontSize: "clamp(32px,7vw,80px)", color: "#fff", letterSpacing: "-0.02em", lineHeight: 1, margin: 0 }}>
            {HEADING.map((word, i) => (
              <span key={i} style={{ display: "block", overflow: "hidden", lineHeight: 1.08 }}>
                <span style={{
                  display: "block",
                  transition: `opacity .75s cubic-bezier(.16,1,.3,1) ${80 + i * 90}ms, transform .75s cubic-bezier(.16,1,.3,1) ${80 + i * 90}ms`,
                  opacity: visible ? 1 : 0,
                  transform: visible ? "none" : "translateY(108%)",
                }}>
                  {word}
                </span>
              </span>
      ...
      <div style={{ position: "relative", height: "1px" }}>
        <div style={{
          ...
          transition: "transform .9s cubic-bezier(.16,1,.3,1) 220ms",
            ...
            >
              ...
              <div className="flex items-start gap-4 lg:gap-[clamp(20px,3vw,60px)] py-6 lg:py-7">
                ...
                <div className="flex flex-col flex-1 min-w-0">
                  <span className="fontone font-bold leading-none text-[clamp(2.2rem,5vw,3.8rem)]"
                    style={{
      ...
      {/* Bottom strip */}
      <div className="mt-14 lg:mt-16 flex items-center justify-between flex-wrap gap-4"
        style={{ transition: "opacity .7s ease 740ms, transform .7s cubic-bezier(.16,1,.3,1) 740ms", opacity: visible ? 1 : 0, transform: visible ? "none" : "translateY(22px)" }}>
        <span className="robo text-[0.65rem] tracking-[0.2em] txtdgray uppercase">Est. 2026 — Global</span>
        <span className="robo flex items-center gap-2 text-[0.65rem] tracking-[0.12em] txtdgray uppercase">
          <span className="w-[6px] h-[6px] rounded-full bg-[#b4ff3c]" style={{ animation: "abpulse 2s ease-in-out infinite" }} />
          Accepting new clients
        </span>
      </div>

      <style>{`@keyframes abpulse{0%,100%{opacity:1;transform:scale(1)}50%{opacity:.35;transform:scale(.65)}}`}</style>
    </section>
  );
};
